...
class BaseModel(torch.nn.Module):
	...
	def loss(self, pred, true_label, original_score = None, pos_neg_ent=None):
		...
		if self.p.loss_func == 'bce':
		...
		elif self.p.loss_func == 'marginRank':
			if self.invest == 1:
				print('loss function: Margin Ranking Loss')
				
			if self.p.use_all_neg_samples == False and self.p.neg_num == 0:
				raise ValueError('margin ranking loss use the negative sampling')
			...
			# margin = torch.Tensor([self.margin]).to(p_score.device)
			# loss = (torch.max(n_score - p_score.view(-1,1), -margin)).mean() + margin
			

			p_score = p_score.unsqueeze(1).repeat(1,n_score.size(-1)).reshape(-1,1)
			n_score = n_score.reshape(-1, 1)
			target = torch.ones_like(p_score, device=p_score.device)

			loss = F.margin_ranking_loss(p_score, n_score, target, margin=self.margin, reduction='mean')

		elif self.p.loss_func == 'bpr':
			if self.invest == 1:
				print('loss function: BPR Loss')
				

			if self.p.use_all_neg_samples == False and self.p.neg_num == 0:
				raise ValueError('bpr loss use the negative sampling')
			...
			# print(n_score.size(), p_score.size())
			# print('p score: ', p_score)
			# print('n score: ', n_score)
			# print((n_score - p_score.view(-1,1)))
			loss = torch.mean(torch.nn.functional.softplus(n_score - p_score.view(-1,1)))
		
		elif self.p.loss_func == 'logsoftmax':
			if self.invest == 1:
				print('loss function: logsoftmax')
				

			if self.p.use_all_neg_samples == False and self.p.neg_num == 0:
				raise ValueError('logsoftmax loss use the negative sampling')

			# pred = torch.sigmoid(pred)
			# print(pred.size(), pos_neg_ent.size())
			if pos_neg_ent != None:

				label_pos = pos_neg_ent[:,0]
				# print(original_score.size())

				loss = self.logsoftmax_loss(original_score, label_pos)
			else:  ####test
				loss = None
